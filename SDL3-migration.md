...
## FreeSerf SDL3 Migration - Pixel Format Standardization

### Color Channel Issues and Fix

During the SDL2 to SDL3 migration, color channels (red/blue) appeared swapped in game assets. This was caused by inconsistent pixel format usage between the sprite loading and video systems.

**Root Cause**: 
- `sprite-file-sdl.cc` used different pixel formats for SDL2 (`SDL_PIXELFORMAT_ARGB8888`) vs SDL3 (`SDL_PIXELFORMAT_RGBA8888`)
- `video-sdl.cc` always expected `SDL_PIXELFORMAT_RGBA8888`
- This mismatch caused ARGB→RGBA channel ordering confusion

**Fix Applied**:
- Standardized `sprite-file-sdl.cc` to use `SDL_PIXELFORMAT_RGBA8888` for both SDL2 and SDL3
- This ensures consistent pixel format throughout the pipeline

**Files Modified**:
- `src/sprite-file-sdl.cc`: Removed SDL version-specific pixel format branching, now uses RGBA8888 universally

**Testing**:
- Color channel debugging system (`-c -s` flags) can be used to verify correct color rendering
- All unit tests pass with the standardized pixel format
